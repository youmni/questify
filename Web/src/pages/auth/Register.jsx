...
const Register = () => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (form.firstName.trim().length < 1) newErrors.firstName = "Voornaam is vereist";
    if (form.lastName.trim().length < 1) newErrors.lastName = "Achternaam is vereist";
    if (!form.email || !/\S+@\S+\.\S+/.test(form.email)) newErrors.email = "Geldig e-mailadres is vereist";
    if (!form.password || !passwordRegex.test(form.password)) newErrors.password = "Min. 8 tekens, 1 hoofdletter, 1 kleine letter, 1 cijfer, 1 speciaal teken";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage("Registratie gelukt! Controleer je e-mail om je account te activeren.");
      setForm({ firstName: "", lastName: "", email: "", password: "", confirmPassword: "" });
    } catch (error) {
      setErrors({ submit: error?.response?.data?.message || "Registratie mislukt" });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className={`w-full max-w-lg relative z-10 opacity-0 ${isVisible ? 'animate-slide-up' : ''}`}>
          <div className="bg-white border border-[#2c3e54]/10 rounded-3xl p-8 shadow-[0_10px_40px_rgba(44,62,84,0.05)]">
            ...
            <div className="text-center mb-8">
              ...
              <h2 className="text-3xl font-bold text-[#2c3e54] mb-2">Account aanmaken</h2>
              <p className="text-[#2c3e54]/60">Doe mee aan de queeste.</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-sm font-medium text-[#2c3e54] ml-1">Voornaam</label>
                  <div className="relative group">
                    ...
                    <input name="firstName" type="text" value={form.firstName} onChange={handleChange} placeholder="Jan"
                ...
                <div className="space-y-1">
                  <label className="text-sm font-medium text-[#2c3e54] ml-1">Achternaam</label>
                  <div className="relative group">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#2c3e54]/40 group-focus-within:text-[#2c3e54] transition-colors"><UserIcon /></div>
                    <input name="lastName" type="text" value={form.lastName} onChange={handleChange} placeholder="de Vries"
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-[#2c3e54] ml-1">E-mailadres</label>
                <div className="relative group">
                  ...
                  <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="verkenner@questify.com"
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-[#2c3e54] ml-1">Wachtwoord</label>
                <div className="relative group">
                  ...
                  <input name="password" type="password" value={form.password} onChange={handleChange} placeholder="Min. 8 tekens, 1 speciaal teken"
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-[#2c3e54] ml-1">Wachtwoord bevestigen</label>
                <div className="relative group">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#2c3e54]/40 group-focus-within:text-[#2c3e54] transition-colors"><LockIcon /></div>
                  <input name="confirmPassword" type="password" value={form.confirmPassword} onChange={handleChange} placeholder="Herhaal wachtwoord"
              ...
              <button type="submit" disabled={isLoading}
                ...
                {isLoading ? (<><LoaderIcon /> Bezig met registreren...</>) : (<>Account aanmaken <ArrowRightIcon /></>)}
              </button>
            </form>
