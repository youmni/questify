...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccessMessage("Als het e-mailadres bekend is, ontvang je binnenkort een resetlink.");
      setEmail("");
    } catch (error) {
      setErrors({ submit: error?.response?.data?.message || "Verzoek mislukt" });
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className={`w-full max-w-md relative z-10 opacity-0 ${isVisible ? 'animate-slide-up' : ''}`}>
          <div className="bg-white border border-[#2c3e54]/10 rounded-3xl p-8 shadow-[0_10px_40px_rgba(44,62,84,0.05)]">
            ...
            <div className="text-center mb-8">
              ...
              <h2 className="text-3xl font-bold text-[#2c3e54] mb-2">Wachtwoord vergeten</h2>
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              >
                {isLoading ? (
                    <><LoaderIcon /> Bezig met verzenden...</>
                ) : (
                    <>Resetlink versturen <ArrowRightIcon /></>
                )}
              </button>
